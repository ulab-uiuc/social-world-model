...
def parse_args():
    ...
    parser.add_argument(
        '--input_file', type=str, required=True, help='Input flat format data with news'
    )
    parser.add_argument(
        '--output_file',
        type=str,
...
"""
    ...
    return _call_llm_for_scores(
        client, model, prompt, news_batch, start_idx, max_retries
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if len(scores) != len(news_batch):
                raise ValueError(
                    f'Score count mismatch: got {len(scores)}, expected {len(news_batch)}'
                )
...
def main():
    ...
    if args.output_file is None:
        ...
        args.output_file = str(
            input_path.parent / f'{input_path.stem}_attributed{input_path.suffix}'
        )
    ...
    breakpoints_with_news = sum(
        1 for s in samples if s.get('sample_type') == 'breakpoint' and s.get('news')
    )
    normal_with_news = sum(
        1 for s in samples if s.get('sample_type') == 'normal_point' and s.get('news')
    )
