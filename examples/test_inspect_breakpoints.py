...
def print_breakpoint(bp, idx, show_news=False):
    ...
    print(f'\n  Breakpoint {idx + 1}:')
    print(
        f"    Before: {format_timestamp(before.get('t'))} | Price: {before.get('p', 'N/A'):.3f}"
    )
    print(
        f"    After:  {format_timestamp(after.get('t'))} | Price: {after.get('p', 'N/A'):.3f}"
    )
    print(
        f"    Change: {bp.get('change', 0):.3f} | Z-score: {bp.get('z_score', 0):.2f}"
    )
    print(
        f"    Window: {format_timestamp(bp.get('window_start'))} to {format_timestamp(bp.get('window_end'))}"
    )
...
def main():
    ...
    print(
        f'  With news: {breakpoints_with_news} ({100*breakpoints_with_news/total_breakpoints:.1f}%)'
    )
    print(
        f'  Empty news []: {breakpoints_empty_news} ({100*breakpoints_empty_news/total_breakpoints:.1f}%)'
    )
    print(
        f'  No news field: {breakpoints_no_field} ({100*breakpoints_no_field/total_breakpoints:.1f}%)'
    )
    print(f'Total news articles: {total_news_articles}')
    if breakpoints_with_news > 0:
        print(
            f'Avg articles per breakpoint (with news): {total_news_articles/breakpoints_with_news:.1f}'
        )
    print('=' * 60)
    ...
    for market in markets:
        ...
        if args.min_news > 0:
            breakpoints = [
                bp for bp in breakpoints if len(bp.get('news', [])) >= args.min_news
            ]
            if not breakpoints:
                continue
        ...
        print(
            f"Question: {market.get('question', market.get('title', 'N/A'))[:100]}..."
        )
