...
def check_file(filepath: str, num_samples: int = 3):
                ...
                for idx, attr in enumerate(attributions):
                    ...
                    if score >= 0.7 and news_idx < len(news):
                        high_score_samples.append({
                            ...
                            'news_description': news[news_idx].get('description', 'N/A'),
    ...
    if window_history_lengths:
        total_len = sum(length * count for length, count in window_history_lengths.items())
    ...
    for ptype in sorted(window_history_by_type.keys()):
        ...
        for length, count in sorted(type_counter.items()):
            ...
            print(f"  length={length}: {count} ({pct:.1f}%)")
        if type_counter:
            type_total_len = sum(length * count for length, count in type_counter.items())
            type_avg = type_total_len / type_total if type_total > 0 else 0
            print(f"  Average: {type_avg:.2f}")
    
    print(f"\n📰 News:")
    print(f"  Samples with news: {has_news} ({has_news/total*100:.1f}%)" if total > 0 else "  N/A")
    if news_counts:
        print(f"  Avg news per sample: {sum(news_counts)/len(news_counts):.1f}")
        print(f"  Min/Max news: {min(news_counts)}/{max(news_counts)}")
    
    print(f"\n🎯 Attributions:")
    print(f"  Samples with attributions: {has_attributions} ({has_attributions/total*100:.1f}%)" if total > 0 else "  N/A")
...
def main():
    ...
    parser.add_argument('--samples', type=int, default=3, help='Number of samples to show (default: 3)')
