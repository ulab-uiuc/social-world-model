...
class PolyMarketDataset(Dataset):
    ...
    def _compute_dataset_hash(self, markets: List[PolyMarketData], window_size: int) -> str:
        hash_content = []
    ...
    def _create_datapoints(self) -> List[Dict[str, torch.Tensor]]:
        ...
        for market in tqdm(self.markets, desc="Creating datapoints"):
            ...
            sim_markets = [
                m for m in self.similar_markets_dict.get(market.market_id, [])
        ...
        encodings = [self.tokenizer(prompt, padding=True, truncation=True, return_tensors='pt') for prompt in tqdm(prompts, desc="Tokenizing prompts")]
    ...
    def _filter_similar_markets(
        ...
        for sim_mkt in similar_markets:
            ...
            sim_mkt.window_series = [
                x for x in sim_mkt.daily_time_series[outcome]
