...
class BasicSocialWMDataset(BaseDataset):
    ...
    def _compute_hash(self) -> str:
        content = [f"{m.market_id}{m.start_ts or ''}{m.end_ts or ''}" 
                  for m in self.markets if m.daily_time_series]
    ...
    def _create_datapoints(self) -> List[Dict[str, torch.Tensor]]:
        ...
        encodings = [self.tokenizer(p, padding=True, truncation=True, return_tensors='pt') 
    ...
    def _create_datapoints(self) -> List[Dict[str, torch.Tensor]]:
        ...
        for market in tqdm(self.markets, desc='Creating datapoints'):
            ...
            for start_idx in range(len(series) - self.window_size):
                ...
                time_overlapped_markets = self._filter_markets(sim_markets, window, target)
                
                prompt = self._build_prompt(market, window, target, time_overlapped_markets)
                prompts.append(prompt)
                metadata.append({
                    'target': target['p'],
                    'market_id': market.market_id,
    ...
    def _filter_markets(self, markets, window_data, target_data):
        ...
        for market in filtered_markets:
            ...
            market.window_series = [
                x for x in market.daily_time_series['Yes']
                if window_start <= x['t'] <= window_end
            ]
        
        return [m for m in filtered_markets if m.window_series]

    def _build_prompt(self, market, window_data, target_data, time_overlapped_markets=None):
