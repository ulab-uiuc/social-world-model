...
def calculate_mape(predictions: List[float], labels: List[float]) -> float:
    return np.mean(np.abs((np.array(labels) - np.array(predictions)) / np.array(labels))) * 100
...
def calculate_metric(predictions: List[float], labels: List[float]) -> dict:
    return {
        ...
        'mape': np.mean(np.abs((np.array(labels) - np.array(predictions)) / np.array(labels))) * 100,
    }