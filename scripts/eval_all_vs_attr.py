...
def _da(records):
    ...
    return sum(
        1 for a, b in zip(pred_delta, true_delta) if sign_tol(a) == sign_tol(b)
    ) / len(records)
...
def main():
    ...
    for p in sorted(ROOT.glob('*_*.jsonl')):
        ...
        attr_recs = [
            r for r in recs if (r['market_id'], hist_fp(r['history'])) in keyset
        ]
    ...
    for split in ('kalshi', 'polymarket'):
        ...
        if sub:
            print(
                f"  {split}: {len(sub)} baselines | n_all={sub[0]['n_all']} n_attr={sub[0]['n_attr']} "
