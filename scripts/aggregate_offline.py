...
def load_scores(path):
    ...
    for l in open(path):
        ...
        d[(r.get('market_id'), t)] = {a['news_idx']: float(a.get('score') or 0)
...
def attr_set(path):
    ...
    for l in open(path):
        d = json.loads(l)
        n = d.get('news') or []
        if any(0 <= x.get('news_idx', -1) < len(n) and float(x.get('score') or 0) > 0
...
def metrics(P, T):
    ...
    mae = np.mean(np.abs(p - t)); b = np.mean(np.abs(t))
...
def main():
    ...
    ap.add_argument('--oracle-file', required=True, help='semdedup test file: defines attr-subset')
    ...
    ap.add_argument('--nonews', default='zero', choices=['zero', 'mu'],
    ...
    for r in rows:
        ...
        if scores_by is not None:
            ...
            sc = {a['news_idx']: float(a.get('score') or 0) for a in (r.get('attributions') or [])}
        ...
        if idxs:
            w = weights([sc[i] for i in idxs], args.scheme, args.rho0, args.eps, args.temp)
        ...
        for sub in (['all', 'attr'] if k in ak else ['all']):
            res[sub][0].append(pred); res[sub][1].append(r['true_delta'])

    tag = args.label or f"{args.scheme}(rho0={args.rho0},T={args.temp},nonews={args.nonews})"
