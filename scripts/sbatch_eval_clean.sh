#!/bin/bash
#SBATCH --job-name=swm-eval-clean
...
#SBATCH --mem=64G
#SBATCH --time=6:00:00
#SBATCH --output=logs/slurm-eval-pm-%j.out
#SBATCH --error=logs/slurm-eval-pm-%j.err
...
CKPT="$REPO/saves/world_model_qwen2p5_3b_clean/final-model"
OUT="$REPO/results/eval_polymarket/clean.jsonl"
mkdir -p "$(dirname "$OUT")"
