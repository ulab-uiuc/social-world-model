...
def greedy_cluster(sim, tau):
    n = len(sim); lab = [-1]*n; c = 0
    for i in range(n):
        if lab[i] >= 0: continue
        lab[i] = c
        for j in range(i+1, n):
            if lab[j] < 0 and sim[i, j] >= tau: lab[j] = c
        c += 1
    return lab
...
for fn in sorted(FILES):
    ...
    for d in recs:
        ...
        s0 = len(texts)
        for n in nw:
            texts.append((n.get("title", "") + ". " + (n.get("description", "") or ""))[:400])
        span.append((s0, len(texts)))
    emb = model.encode(texts, normalize_embeddings=True, batch_size=256,
                       show_progress_bar=False, convert_to_numpy=True) if texts else np.zeros((0, 384))
    tot_old = tot_new = 0
    out = []
    for d, (s0, s1) in zip(recs, span):
        news = d.get("news") or []
        attrs = d.get("attributions") or []
        score_by_idx = {a["news_idx"]: float(a.get("score") or 0)
                        for a in attrs if 0 <= a.get("news_idx", -1) < len(news)}
        tot_old += len(news)
        if len(news) <= 1:
            out.append(d); tot_new += len(news); continue
        E = emb[s0:s1]
        sim = E @ E.T
        lab = greedy_cluster(sim, TAU)
        # per cluster: pick representative = highest posterior score (tie -> lowest idx)
        clusters = {}
        for i, l in enumerate(lab): clusters.setdefault(l, []).append(i)
        ...
        new_news = []; old2newrep = {}; new_scores = {}
        for _, rep, members in reps:
            ni = len(new_news); new_news.append(news[rep])
            mx = max((score_by_idx.get(m, 0.0) for m in members), default=0.0)
            if mx > 0: new_scores[ni] = mx
            for m in members: old2newrep[m] = ni
        d2 = dict(d); d2["news"] = new_news
        d2["attributions"] = [{"news_idx": i, "score": s} for i, s in sorted(new_scores.items())]
        out.append(d2); tot_new += len(new_news)
    with open(os.path.join(DST, fn), "w") as fo:
        fo.write("\n".join(json.dumps(x) for x in out) + "\n")
    print(f"{fn:<28} records={len(recs):>5}  news {tot_old} -> {tot_new}  (-{100*(tot_old-tot_new)/max(tot_old,1):.0f}%)")
print("DONE")
