...
def st(x, t=1e-6): return 1 if x > t else -1 if x < -t else 0
def mets(P, T):
    P = np.array(P); T = np.array(T); mv = np.abs(T) > 1e-6
    mae = np.mean(np.abs(P - T)); base = np.mean(np.abs(T))
    mase = mae / base if base > 0 else float('nan')
    da = np.mean([st(a) == st(b) for a, b in zip(P[mv], T[mv])]) if mv.any() else float('nan')
    ic = np.corrcoef(P, T)[0, 1] if len(P) > 1 and np.std(P) > 0 else 0.0
    return mase, mae, da, ic
def nkey(x): return x.get('url') or (x.get('title', '') + '|' + x.get('description', '')[:40])
...
for plat, tf, pf in [("poly", "test_polymarket_final.jsonl", "test_polymarket_final_ck150.jsonl"),
                     ("kalshi", "test_kalshi_final.jsonl", "test_kalshi_final_ck150.jsonl")]:
    allk = set(); attrk = set(); testnews = {}
    for l in open(f"{DD}/{tf}"):
        d = json.loads(l); k = (d['market_id'], d['target']['t']); allk.add(k)
        testnews[k] = set(nkey(x) for x in (d.get('news') or []))
        if any(float(x.get('score') or 0) != 0 for x in (d.get('attributions') or [])): attrk.add(k)
    ...
    for l in open(f"dumps_ck150_cr/fc8b_{plat}.jsonl"):
        r = json.loads(l); cr[(r['market_id'], r['t'])] = r
    # ck150 file: idx -> news nkey  (to know which ck150 idx is a test news)
    cknews = {}
    for l in open(f"{PD}/{pf}"):
        d = json.loads(l); k = (d['market_id'], (d.get('target') or {}).get('t'))
    ...
    for l in open(f"dumps/fc8b_{plat}.jsonl"):
        r = json.loads(l); duf[(r['market_id'], r['t'])] = {pn['news_idx']: pn['mu'] for pn in r['per_news']}
    ckscore_by_nkey = {}
    for l in open(f"{PD}/{pf}"):
        d = json.loads(l); k = (d['market_id'], (d.get('target') or {}).get('t'))
        nl = d.get('news') or []; sc = {a['news_idx']: float(a.get('score') or 0) for a in (d.get('attributions') or [])}
        ckscore_by_nkey[k] = {nkey(nl[i]): sc.get(i, 0) for i in range(len(nl))}
    testnews_idx = {}
    for l in open(f"{DD}/{tf}"):
        d = json.loads(l); k = (d['market_id'], d['target']['t'])
        testnews_idx[k] = {i: nkey(x) for i, x in enumerate(d.get('news') or [])}

    RES = {m: {'all': ([], []), 'attr': ([], [])} for m in ('A', 'B', 'C')}
    for k in allk:
        if k not in cr: continue
        r = cr[k]; td = r['true_delta']
        mu_ck = {pn['news_idx']: pn['mu'] for pn in r['per_news']}
        sc_ck = {a['news_idx']: float(a.get('score') or 0) for a in (r.get('attributions') or [])}
        ...
        for m, p in (('A', predA), ('B', predB), ('C', predC)):
            for sub in (['all', 'attr'] if k in attrk else ['all']):
                RES[m][sub][0].append(p); RES[m][sub][1].append(td)
    print(f"\n##### {plat} (n_all={len(RES['A']['all'][0])}) #####")
    lab = {'A': 'GPU ck150 全news(零匹配)', 'B': 'GPU ck150 仅test news', 'C': '离线 dumps/+内容匹配(当前表)'}
    print(f"{'方法':>26}{'MASE_all':>9}{'MASE_at':>8}{'DA_all':>7}{'DA_at':>7}{'Corr_all':>9}{'Corr_at':>8}")
    for m in ('A', 'B', 'C'):
        a = mets(*RES[m]['all']); t = mets(*RES[m]['attr'])
        print(f"{lab[m]:>26}{a[0]:>9.3f}{t[0]:>8.3f}{a[2]:>7.3f}{t[2]:>7.3f}{a[3]:>9.3f}{t[3]:>8.3f}")
