...
from swm.utils.prompter import model_prompting
...
def decompose_assumptions(input_file: str, output_file: str):
    ...
    for arxiv_id, introduction in tqdm(paper_introductions.items()):
        ...
        assumptions[arxiv_id]['assumptions'] = (
            parse_assumptions(response[0])
            if response
            else ['No assumptions identified.']
        )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Analyze assumptions from paper introductions.'
    )
    parser.add_argument(
        '--input_file',
        type=str,
        default='../data/iclrbench.json',
        help='Path to the input JSON file containing paper introductions.',
    )
    parser.add_argument(
        '--output_file',
        type=str,
        default='../data/iclrbench_decompose.json',
        help='Path to save the output JSON file with assumptions.',
    )
